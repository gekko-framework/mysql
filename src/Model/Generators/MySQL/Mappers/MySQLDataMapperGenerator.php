...
namespace Gekko\Model\Generators\MySQL\Mappers;
...
class MySQLDataMapperGenerator implements IGenerator
{
    const GEN_CLASS = 1;
    const GEN_TRAIT = 2;
    ...
    private function generateModels(PackageDescriptor $package) : void
    {
        foreach ($package->models as $model)
        {
            ...
            if ($this->config & self::GEN_CLASS)
                \file_put_contents($path . 'DataMapper.php', $this->process("Class", $package, $model));

            if ($this->config & self::GEN_TRAIT)
            {
                \file_put_contents($path . 'DataMapperTrait.php', $this->process("Trait", $package, $model));

                if (!\file_exists($path . 'DataMapper.php'))
                    \file_put_contents($path . 'DataMapper.php', $this->process("TraitUsage", $package, $model));
            }
        }
    }
